...
function CommentObj($http){
	...
	this.addComment = function(rootCommentId,parentId,newComment,callback){
		$http.post('/comments/add',{rootCommentId:rootCommentId,parentCommentId:parentId,newComment:newComment})
		.success(function(data,status,headers,config){
			callback(null,data)
		})
		.error(function(data,status,headers,config){
...
app.controller('pageController',['$scope','$http','commentSrv',function($scope,$http,commentSrv){
	...
	$scope.loadComments = function(){
		commentSrv.getcomment($scope.page.commentsId,function(err,comment){
			if(err){
				$scope.commentThread = {}
			}else{
				$scope.commentThread = comment
